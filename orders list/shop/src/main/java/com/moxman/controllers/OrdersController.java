...
public class OrdersController {
	 ...
	 * public String orderspage(@PathVariable("total") float total, HttpSession
	 * session, Model m) {
	 * 
	 * String email = (String) session.getAttribute("email"); List<Cart> list =
	 * cartdao.retrive(email);
	 * 
	 * Cart ncar = list.get(0); //new Cart(); User user = userdao.getemail(email);
	 * System.out.println(user); float grandtotal = total;
	 * 
	 * Orders norder = new Orders(); norder.setTotal(grandtotal);
	 * norder.setEmail(email);
	 * 
	 * norder.setQuantity(ncar.getQuantity());
	 * System.out.println("---------------Order Quantity---------------"+ncar.
	 * getQuantity()); System.out.println(""); norder.setPimage(ncar.getPimage());
	 * norder.setCitemid(ncar.getCitemid()); norder.setProdname(ncar.getProdname());
	 * norder.setPrice(ncar.getPrice()); norder.setEmail(email);
	 * orderdao.addorder(norder); List<Orders> list1=orderdao.reteriveorders(email);
	 * 
	...
	@RequestMapping(value = "/orders/date/{date}", method = RequestMethod.GET)
	public String batch1(@RequestParam int date, Model m, HttpSession session) {

		String email = (String) session.getAttribute("email");
		User user = userdao.getemail(email);
		if (user.getRole().equals(email)) {

			orderdao.batch1(email);
